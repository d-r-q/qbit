...
actual class Typing<T : Any> actual constructor(
    actual val root: StoredEntity,
    ...
    actual fun <R : Any> instantiate(e: StoredEntity, type: KClass<R>): R {
            ...
            if (!param.type.isMarkedNullable || param.type.classifier in valueTypes ||
                ...
                when {
                    ...
                    param.type.classifier == List::class -> {
                        ...
                        val referedEntity = entities[e[attr] as Gid]
                            ?: throw QBitException("There is no entity for gid: ${e[attr]}, attr: $attr")
            ...
            if (attr != null) {
                ...
                if (propType?.value() == true) {
                    ...
                    if (propType?.isList() != true) {
                        ...
                        val instances = e.getGidList(attr).map { gid ->
                            instantiate(
                                entities[gid]!!,
                                prop.returnType.arguments[0].type!!.classifier as KClass<*>
                            )
                        }
