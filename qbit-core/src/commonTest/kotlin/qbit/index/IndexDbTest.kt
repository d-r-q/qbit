...
class DbTest {
    ...
    fun testSearchByAttrRangeAndAttrValue() {
        runBlocking {
            ...
            val root =
                Root(Hash(ByteArray(20)), dbUuid, time1, NodeData((bootstrapSchema.values.flatMap { it.toFacts() } +
            ...
            assertArrayEquals(
                arrayOf(Gid(pChen.id!!)),
                db.query(attrIn(extId, 1, 3), attrIs(name, "Peter Chen")).map { it.gid }.toList().toTypedArray()
            )
        }
    }
    ...
    fun `Entity with multiple values of list attribute should be returned from query only once`() {
        runBlocking {
            ...
            val db = TestIndexer().index(root)
            assertArrayEquals(
                arrayOf(Gid(eCodd.id!!)),
                db.query(attrIn(nicks, "n", "u")).map { it.gid }.toList().toTypedArray()
            )
        }
    }
    ...
    fun `Indexer can index multiple transactions`() {
        runBlocking {
            ...
            val n1 = Leaf(
                ...
                dbUuid,
                currentTimeMillis(),
                NodeData(pChen.toFacts().toList().toTypedArray())
            )
            nodes[n1.hash] = n1
            ...
            val n2 = Leaf(
                ...
                dbUuid,
                currentTimeMillis(),
                NodeData(mStonebreaker.toFacts().toList().toTypedArray())
            )
            nodes[n2.hash] = n2
    ...
    fun `Indexer can index updates`() {
        runBlocking {
            ...
            val root = Root(
                ...
                dbUuid,
                currentTimeMillis(),
                NodeData((extId.toFacts() + name.toFacts() + nicks.toFacts() + eCodd.toFacts()).toTypedArray())
            )
            val nodes = hashMapOf<Hash, NodeVal<Hash>>(root.hash to root)
            val nodeResolver = mapNodeResolver(nodes)
            var db = TestIndexer(nodeResolver = nodeResolver).index(root)
            ...
            val n2 = Leaf(
                ...
                dbUuid,
                currentTimeMillis(),
                NodeData(pChen.copy(externalId = 5).toFacts().toList().toTypedArray())
            )
            nodes[n2.hash] = n2
    ...
    fun `pull with fetch = Eager should fetch nullable refs`() {
        runBlocking {
            ...
            val root = Root(
                Hash(ByteArray(20)), dbUuid, currentTimeMillis(), NodeData(
                    (extId.toFacts() + name.toFacts() + nicks.toFacts() + reviewer.toFacts() + country.toFacts() +
